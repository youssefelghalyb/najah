    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('bundles.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-xl transition-all shadow-lg shadow-red-500/30">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-4">
            <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search bundles..." class="px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500">
        ...
        @if($bundles->count() > 0)
            <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                @foreach($bundles as $bundle)
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-xl transition-all">
                        <!-- Checkbox -->
                        <div class="absolute top-4 left-4 z-10">
                            <input type="checkbox" value="{{ $bundle->id }}" class="bundle-checkbox w-5 h-5 text-red-600 bg-white border-gray-300 rounded focus:ring-2 focus:ring-red-500" onchange="updateBulkActions()">
                        </div>
                        ...
                        <div class="relative bg-gradient-to-br from-red-500 to-red-600 p-6">
                            <div class="absolute top-4 right-4">
                                @if($bundle->status === 'active')
                                    <span class="px-2.5 py-1 bg-white/20 backdrop-blur-sm text-white text-xs font-bold rounded-lg">Active</span>
                                @else
                                    <span class="px-2.5 py-1 bg-white/20 backdrop-blur-sm text-white text-xs font-bold rounded-lg">Inactive</span>
                                @endif
                            </div>
                        ...
                        <div class="p-6">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach($bundle->products->take(3) as $product)
                                        <span class="px-2.5 py-1 bg-gray-100 text-gray-700 text-xs font-medium rounded-lg">
                                            {{ $product->name }}
                                        </span>
                                    @endforeach
                                    @if($bundle->products_count > 3)
                                        <span class="px-2.5 py-1 bg-gray-100 text-gray-500 text-xs font-medium rounded-lg">
                            ...
                            <div class="mb-4 pb-4 border-b border-gray-200">
                                <div class="flex items-center justify-between mb-2">
                                    ...
                                    <span class="text-sm font-semibold text-gray-400 line-through">${{ number_format($bundle->total_price, 2) }}</span>
                                </div>
                                <div class="flex items-center justify-between">
                                    <span class="text-sm font-bold text-gray-900">Bundle Price:</span>
                                    <span class="text-2xl font-black text-red-600">${{ number_format($bundle->final_price, 2) }}</span>
                                </div>
                                @if($bundle->hasDiscount())
                                    <div class="flex items-center justify-end mt-1">
                                        <span class="text-xs font-bold text-green-600">Save {{ number_format($bundle->discount_percentage) }}% ({{ $bundle->discount_type === 'percentage' ? $bundle->discount_amount . '%' : '$' . number_format($bundle->discount_amount, 2) }})</span>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="flex items-center gap-2">
                                <a href="{{ route('bundles.show', $bundle) }}" class="flex-1 px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium rounded-lg text-center transition-all text-sm">
                                    View
                                </a>
                                <a href="{{ route('bundles.edit', $bundle) }}" class="flex-1 px-3 py-2 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg text-center transition-all text-sm">
                                    Edit
                                </a>
                            </div>
                        </div>
        ...
        @else
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-12 text-center">
                ...
                <a href="{{ route('bundles.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-xl transition-all">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                    </svg>
                    Create Bundle
        ...
        async function bulkUpdateStatus(status) {
            ...
            try {
                const response = await fetch('{{ route("bundles.bulk-update-status") }}', {
                    ...
                    body: JSON.stringify({ bundles: ids, status: status })
                });
        ...
        async function bulkDelete() {
            ...
            try {
                const response = await fetch('{{ route("bundles.bulk-delete") }}', {
                    ...
                    body: JSON.stringify({ bundles: ids })
                });
