        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-4">
            <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-3">
                ...
                <select name="return_status"
                    ...
                    <option value="pending" {{ request('return_status') == 'pending' ? 'selected' : '' }}>Pending
                    </option>
                    <option value="approved" {{ request('return_status') == 'approved' ? 'selected' : '' }}>Approved
                    </option>
                    <option value="rejected" {{ request('return_status') == 'rejected' ? 'selected' : '' }}>Rejected
                    </option>
                    <option value="completed" {{ request('return_status') == 'completed' ? 'selected' : '' }}>Completed
                    </option>
                </select>
                <button type="submit"
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach ($orders as $order)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="text-sm font-semibold text-gray-900">{{ $order->items->count() }}
                                            items</span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <span class="text-sm font-black text-gray-900">EGP
                                    ...
                                    <td class="px-6 py-4">
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 {{ $order->payment_status_badge_class }} text-xs font-bold rounded-lg">
                                    ...
                                    <td class="px-6 py-4">
                                        @if ($order->return_status)
                                            <span
                                                class="inline-flex items-center px-2.5 py-1 {{ $order->return_status_badge_class }} text-xs font-bold rounded-lg">
                                    ...
                                    <td class="px-6 py-4">
                                        @if ($order->hasAllQRCodesAssigned())
                                            <span
                                                class="inline-flex items-center gap-1 px-2.5 py-1 bg-green-100 text-green-800 text-xs font-bold rounded-lg">
