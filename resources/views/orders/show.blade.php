    ...
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                @if($order->canAssignQRCodes())
                    <a href="{{ route('orders.assign-qr-codes', $order) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-all text-sm">
                ...
                <a href="{{ route('orders.edit', $order) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-gray-900 hover:bg-gray-800 text-white font-medium rounded-lg transition-all text-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-start justify-between">
                ...
                <div class="flex items-center gap-3">
                    <span class="inline-flex items-center px-4 py-2 {{ $order->status_badge_class }} text-sm font-bold rounded-lg">
                        {{ $order->status_label }}
                    </span>
                    <span class="inline-flex items-center px-4 py-2 {{ $order->payment_status_badge_class }} text-sm font-bold rounded-lg">
                            ...
                            <div class="flex items-start gap-4 p-4 bg-gray-50 rounded-xl border border-gray-200">
                                <div class="flex-1">
                                    ...
                                    <div class="flex items-center gap-4 text-sm text-gray-600">
                                        ...
                                        <span>Unit Price: <strong class="text-gray-900">EGP {{ number_format($item->unit_price, 2) }}</strong></span>
                                    </div>
                                        ...
                                        @if($item->qrCode)
                                            <div class="flex items-center justify-between">
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    <span class="text-sm font-semibold text-green-800">QR Code: {{ $item->qrCode->code }}</span>
                                                </div>
                                    ...
                                    <div class="flex items-start gap-4">
                                        ...
                                        <div class="flex-1">
                                            <div class="flex items-center justify-between">
                                                ...
                                                <span class="text-xs text-gray-500">{{ $history->changed_at->format('M d, Y h:i A') }}</span>
                                            </div>
                                            @if($history->notes)
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-3 text-sm">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="font-semibold text-gray-900">EGP {{ number_format($order->subtotal, 2) }}</span>
                        </div>
                        @if($order->discount_amount > 0)
                            <div class="flex items-center justify-between text-green-600">
                                <span>Discount:</span>
                                <span class="font-semibold">-EGP {{ number_format($order->discount_amount, 2) }}</span>
                            </div>
                        @endif
                        @if($order->tax_amount > 0)
                            <div class="flex items-center justify-between">
                                <span class="text-gray-600">Tax:</span>
                                <span class="font-semibold text-gray-900">EGP {{ number_format($order->tax_amount, 2) }}</span>
                            </div>
                        @endif
                        @if($order->shipping_amount > 0)
                            <div class="flex items-center justify-between">
                                <span class="text-gray-600">Shipping:</span>
                                <span class="font-semibold text-gray-900">EGP {{ number_format($order->shipping_amount, 2) }}</span>
                            </div>
                        @endif
                        <div class="pt-3 border-t border-gray-200 flex items-center justify-between">
                            <span class="font-bold text-gray-900">Total:</span>
                            <span class="text-2xl font-black text-red-600">EGP {{ number_format($order->total, 2) }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-2">
                        @if($order->status === 'pending' && $order->hasAllQRCodesAssigned() && $order->payment_status === 'paid')
                            <button onclick="changeStatus('processing')" class="w-full px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-all text-sm">
                                Move to Processing
                            </button>
                        @endif
                        
                        @if($order->status === 'processing')
                            <button onclick="changeStatus('shipped')" class="w-full px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white font-medium rounded-lg transition-all text-sm">
                                Mark as Shipped
                            </button>
                        @endif
                        
                        @if($order->status === 'shipped')
                            <button onclick="changeStatus('delivered')" class="w-full px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg transition-all text-sm">
                                Mark as Delivered
                            </button>
                        @endif
                        
                        @if(in_array($order->status, ['pending', 'processing']))
                            <button onclick="changeStatus('cancelled')" class="w-full px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg transition-all text-sm">
                                Cancel Order
                            </button>
                        @endif
                    </div>
